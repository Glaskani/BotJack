...
module.exports = new Command("jack", function(message) {
	  ...
	  let usage = 'Usage: `play <meme>` play a meme\n'
							+ '       `play meme` list of disponible meme\n'
	  ...
	  let guild = message.guild;
		async function play() {
				...
				else if (args < 1) {Print.reply('Argument missing', message);return;} //Args missings
				else if (args > 2) {Print.reply('Too much argument given', message);return;} //Args missings
				//Main methode
				if (args[0]=="volume" && args.length === 2) {
						...
						Print.embed('The disponible meme are:\n', message);
